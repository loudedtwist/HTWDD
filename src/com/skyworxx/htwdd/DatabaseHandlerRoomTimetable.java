...
public class DatabaseHandlerRoomTimetable extends SQLiteOpenHelper {
	...
	// Database Name
	private static final String DATABASE_NAME = "RaumManager";
	...
	public DatabaseHandlerRoomTimetable(Context context) {
	...
	public Type_Stunde getStunde(String tag, int woche, int stundee) {
		...
		String selectQuery = "SELECT  * FROM " + TABLE_STUNDEN
				...
				+ " AND STUNDE =" + stundee + " ORDER BY STUNDE";
	...
	public List<Type_Stunde> getStundenTag(String tag, int woche, String raum) {

		List<Type_Stunde> stundenList = new ArrayList<Type_Stunde>();
		// Select All Query
		String selectQuery = "SELECT * FROM " + TABLE_STUNDEN
				+ " WHERE TAG = '" + tag + "' AND WOCHE = "
				+ Integer.toString(woche) + " AND RAUM = '" + raum+"'"+ " ORDER BY STUNDE";
	...
	public List<Type_Stunde> getDiffRaum() {
		...
		if (cursor.moveToFirst()) {
			do {
				...
				// Adding contact to list
				if (type_Stunde.getRaum().length()!=0 && !type_Stunde.getRaum().contains(";"))
				stundenList.add(type_Stunde);
			} while (cursor.moveToNext());
		}
		cursor.close();
	...
	// Getting contacts Count
		public int getRaumCount(String raum) {

			String countQuery = "SELECT  * FROM " + TABLE_STUNDEN+" WHERE RAUM = '"+raum+"'";
			SQLiteDatabase db = this.getReadableDatabase();
	...
	public int updateStunde(Type_Stunde type_Stunde) {
		...
		// updating row
		return db.update(TABLE_STUNDEN, values, KEY_ID + " = ?",
	...
	public void overwriteStunde(Type_Stunde type_Stunde) {
		...
		String selectQuery = "Delete FROM " + TABLE_STUNDEN + " WHERE TAG = '"
				...
				+ " AND STUNDE = " + type_Stunde.getStunde()+ " AND RAUM = '" + type_Stunde.getRaum()+"'";
